...
export async function uploadRelease(client: AxiosInstance, config: ReleaseConfig) {
  ...
  for (const platformEntry of updatedPlatformEntries) {
    ...
    const urls = await getSignedUploadUrls(
      ...
      files.map(file => ({
        ...file,
        fileType: "application/octet-stream", // Adding the missing fileType property
      })),
      { client }
    );
    CliUx.ux.action.stop();

    // Upload each file (in WebGL folder or single file)
...
// Helper function for non-WebGL platforms
